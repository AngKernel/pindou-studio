...
export async function downloadImage({
  ...
  const processDownload = () => {
    ...
    let downloadHeight = titleBarHeight + gridHeight + (axisLabelSize * 2) + statsHeight + extraTopMargin + extraBottomMargin + footerAreaHeight;
    ...
    ctx.fillText('Pindou Studio', titleStartX, mainTitleY);
    ...
    const secondaryText = 'Pindou Studio';
    ...
    if (includeStats && colorCounts) {
      ...
      const statsWatermarkText = '图纸生成：Pindou Studio';
    ...
    if (includeStats && colorCounts) {
      ...
      const newDownloadHeight = titleBarHeight + extraTopMargin + M * downloadCellSize + (axisLabelSize * 2) + statsHeight + extraBottomMargin + footerAreaHeight;
